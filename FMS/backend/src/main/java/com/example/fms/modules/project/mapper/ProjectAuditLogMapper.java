...
public interface ProjectAuditLogMapper {
    ...
    @Select("SELECT l.id, l.action, CASE l.node_code WHEN 'UNIT_REVIEW' THEN 1 WHEN 'FINANCE_REVIEW' THEN 2 WHEN 'APPROVED' THEN 3 WHEN 'REJECTED' THEN 4 WHEN 'CLOSED' THEN 5 WHEN 'DISABLED' THEN 6 WHEN 'CREATE' THEN 0 ELSE NULL END AS fromStatus, CASE l.node_code WHEN 'UNIT_REVIEW' THEN 1 WHEN 'FINANCE_REVIEW' THEN 2 WHEN 'APPROVED' THEN 3 WHEN 'REJECTED' THEN 4 WHEN 'CLOSED' THEN 5 WHEN 'DISABLED' THEN 6 WHEN 'CREATE' THEN 0 ELSE NULL END AS toStatus, l.actor_user_id AS operatorUserId, u.real_name AS operatorName, l.comment, DATE_FORMAT(l.created_at, '%Y-%m-%d %H:%i:%s') AS createdAt FROM wf_log l LEFT JOIN sys_user u ON u.id = l.actor_user_id WHERE l.biz_type = 'PROJECT' AND l.biz_id = #{projectId} AND l.node_code IN ('CREATE','SUBMIT','UNIT_REVIEW','FINANCE_REVIEW','APPROVED','REJECTED','CLOSED','DISABLED','WITHDRAWN','DELETED','MEMBER') ORDER BY l.id ASC")
