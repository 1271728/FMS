...
public interface ReimburseAuditLogMapper {
    ...
    @Select("SELECT l.id, l.action, CASE l.node_code WHEN 'PI_AUDIT' THEN 7 WHEN 'UNIT_AUDIT' THEN 1 WHEN 'FIN_REVIEW' THEN 2 WHEN 'PAY_ARCHIVE' THEN 3 WHEN 'DONE' THEN 4 WHEN 'REJECTED' THEN 5 WHEN 'WITHDRAWN' THEN 6 ELSE NULL END AS fromStatus, CASE l.node_code WHEN 'PI_AUDIT' THEN 7 WHEN 'UNIT_AUDIT' THEN 1 WHEN 'FIN_REVIEW' THEN 2 WHEN 'PAY_ARCHIVE' THEN 3 WHEN 'DONE' THEN 4 WHEN 'REJECTED' THEN 5 WHEN 'WITHDRAWN' THEN 6 ELSE NULL END AS toStatus, l.actor_user_id AS operatorUserId, su.real_name AS operatorName, l.comment, DATE_FORMAT(l.created_at, '%Y-%m-%d %H:%i:%s') AS createdAt FROM wf_log l LEFT JOIN sys_user su ON su.id = l.actor_user_id WHERE l.biz_type = 'REIMB' AND l.biz_id = #{reimburseId} AND l.node_code IN ('CREATE','PI_AUDIT','UNIT_AUDIT','FIN_REVIEW','PAY_ARCHIVE','DONE','REJECTED','WITHDRAWN') ORDER BY l.id ASC")
